...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:gml="http://www.opengis.net/gml" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:datacite="http://datacite.org/schema/kernel-4" xmlns:tr="java:org.fao.geonet.api.records.formatters.SchemaLocalizations" xmlns:saxon="http://saxon.sf.net/" xmlns:gn="http://www.fao.org/geonetwork" version="2.0" extension-element-prefixes="saxon" exclude-result-prefixes="#all">
  ...
  <xsl:param name="doiPrefix" select="'10.5072'" />
  ...
  <!--
       ...
       property.
  -->
  <xsl:variable name="creatorRoles" select="'owner', 'originator', 'principalInvestigator', 'author'" />
  <xsl:variable name="publisherRoles" select="'resourceProvider', 'custodian', 'distributor', 'pointOfContact', 'processor', 'publisher'" />
  <!--<datacite:publisher>
       <xsl:value-of select="$metadata//gmd:distributor[1]/*/gmd:distributorContact[1]/*/gmd:organisationName/gco:CharacterString" />
       </datacite:publisher>-->
  <xsl:template mode="toDatacite" match="gmd:MD_Metadata/gmd:identificationInfo/*/gmd:pointOfContact[1]">
    <datacite:publisher>
      <!--<xsl:value-of select="$metadata//gmd:distributor[1]/*/gmd:distributorContact[1]/*/gmd:organisationName/gco:CharacterString" />-->
      <xsl:value-of select="../gmd:pointOfContact/gmd:role/*[@codeListValue = 'pointOfContact']/../../gmd:organisationName/gco:CharacterString" />
    </datacite:publisher>
    <datacite:creators>
          ...
          <xsl:choose>
            <xsl:when test="gmd:individualName">
              <datacite:creatorName nameType="Personal">
                <xsl:value-of select="gmd:individualName/gco:CharacterString/text()" />
              </datacite:creatorName>
            </xsl:when>
            <xsl:otherwise>
              <datacite:creatorName nameType="Organizational">
                <xsl:value-of select="gmd:organisationName/gco:CharacterString/text()" />
              </datacite:creatorName>
            </xsl:otherwise>
          </xsl:choose>
