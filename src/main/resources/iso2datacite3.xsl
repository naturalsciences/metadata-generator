...
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:gml="http://www.opengis.net/gml" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:datacite="http://datacite.org/schema/kernel-4" xmlns:tr="java:org.fao.geonet.api.records.formatters.SchemaLocalizations" xmlns:saxon="http://saxon.sf.net/" xmlns:gn="http://www.fao.org/geonetwork" version="2.0" extension-element-prefixes="saxon" exclude-result-prefixes="#all">
  ...
  <xsl:param name="doiPrefix" select="'doiPrefixPlaceholder'" />
  ...
  <xsl:variable name="metadata" select="//gmd:MD_Metadata" />
  <xsl:variable name="publisherRoles" select="'custodian', 'distributor', 'publisher'" />
  <xsl:variable name="creatorRoles" select="'author', 'originator','owner', 'principalInvestigator', 'resourceProvider'" />
  <xsl:variable name="contactRoles" select="'pointOfContact'" />
  <xsl:variable name="datamanagerRoles" select="'processor'" />
  <!-- TODO: Convert language code eng > en_US ? -->
  <xsl:variable name="metadataLanguage" select="//gmd:MD_Metadata/gmd:language/*/@codeListValue" />
  <xsl:template match="/">
    <datacite:resource xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://datacite.org/schema/kernel-4 http://schema.datacite.org/meta/kernel-4.1/metadata.xsd">
      <xsl:apply-templates select="$metadata" mode="toDatacite" />
      <datacite:contributors>
        <xsl:for-each select="//gmd:CI_ResponsibleParty[gmd:role/*[@codeListValue = $contactRoles]]/gmd:individualName/gco:CharacterString">
        ...
        <xsl:for-each select="//gmd:CI_ResponsibleParty[gmd:role/*[@codeListValue = $datamanagerRoles]]/gmd:individualName/gco:CharacterString">
          <xsl:element name="datacite:contributor">
            <xsl:attribute name="contributorType" select="'DataManager'" />
            <datacite:contributorName>
              <xsl:value-of select="." />
            </datacite:contributorName>
            <datacite:affiliation>
      ...
      <xsl:for-each select="//gmd:CI_ResponsibleParty[gmd:role/*[@codeListValue = $publisherRoles]]">
        <xsl:sort select="gmd:role/*/@codeListValue" order="descending" />
        <xsl:if test="position()=1">
          <xsl:variable name="publisherInstitution" select="gmd:organisationName/gco:CharacterString" />
          <datacite:publisher>
            <xsl:value-of select="$publisherInstitution" />
          </datacite:publisher>
        </xsl:if>
      </xsl:for-each>
      <datacite:creators>
        <xsl:for-each select="distinct-values(//gmd:CI_ResponsibleParty[gmd:role/*[@codeListValue = $creatorRoles]]/gmd:organisationName/gco:CharacterString)">
          <datacite:creator>
            ...
            <!-->Unfortunately you can't mix a distinct node with getting anything with that node as a context<-->
          </datacite:creator>
        </xsl:for-each>
        <xsl:for-each select="distinct-values(//gmd:CI_ResponsibleParty[gmd:role/*[@codeListValue = $creatorRoles]]/gmd:individualName/gco:CharacterString)">
          <datacite:creator>
  ...
  <!--
       ...
       versions.
  -->
  <xsl:template mode="toDatacite" match="gmd:MD_Metadata/gmd:fileIdentifier/*/text()">
  ...
  -->
  <xsl:template mode="toDatacite" match="gmd:MD_Metadata/gmd:identificationInfo/*/gmd:language[1]/*/@codeListValue">
  ...
  -->
  <xsl:template mode="toDatacite" match="gmd:MD_Metadata/gmd:identificationInfo/*/gmd:citation/*/gmd:title">
    <datacite:titles>
  ...
  -->
  <xsl:template mode="toDatacite" match="gmd:MD_Metadata/gmd:identificationInfo/*/gmd:abstract">
  ...
  <xsl:template mode="toDatacite" match="gmd:EX_GeographicBoundingBox[1]">
  ...
  <xsl:template mode="toDatacite" match="gmd:MD_Metadata/gmd:identificationInfo/*/gmd:descriptiveKeywords[1]">
    <datacite:subjects>
      <xsl:for-each select="$metadata//gmd:keyword">
        <xsl:call-template name="toDataciteLocalized">
          <xsl:with-param name="template">
            <datacite:subject>
              <xsl:variable name="thesaurusTitle" select="../gmd:thesaurusName/*/gmd:title" />
              <xsl:if test="$thesaurusTitle/gmx:Anchor/@xlink:href">
                <xsl:attribute name="schemeURI" select="$thesaurusTitle/gmx:Anchor/@xlink:href" />
              </xsl:if>
              <xsl:if test="$thesaurusTitle/*/text()">
                <xsl:attribute name="subjectScheme" select="normalize-space($thesaurusTitle/(gco:CharacterString|gmx:Anchor)/text()[. != ''])" />
              </xsl:if>
              <!--<xsl:if test="gmx:Anchor/@xlink:href">
                   <xsl:attribute name="valueUri" select="gmx:Anchor/@xlink:href" />
                   </xsl:if>-->
              <!--Illegal for DataCite-->
            </datacite:subject>
          </xsl:with-param>
        </xsl:call-template>
      </xsl:for-each>
  ...
  <xsl:template mode="toDatacite" match="gmd:hierarchyLevel/*/@codeListValue">
  ...
  -->
  <xsl:template mode="toDatacite" match="gmd:MD_Metadata/gmd:identificationInfo/*/gmd:citation/*/gmd:edition">
    <datacite:version>
  ...
  -->
  <xsl:template mode="toDatacite" match="gmd:useLimitation[1]">
    <datacite:rightsList>
      <xsl:for-each select="$metadata//gmd:useLimitation">
        <xsl:apply-templates mode="toDataciteLocalized" select=".">
          <xsl:with-param name="template">
            <datacite:rights>
              <xsl:if test="gmx:Anchor/@xlink:href">
                <xsl:attribute name="rightsURI" select="gmx:Anchor/@xlink:href" />
              </xsl:if>
            </datacite:rights>
          </xsl:with-param>
        </xsl:apply-templates>
  ...
  -->
  <xsl:template mode="toDatacite" match="gmd:distributionInfo">
    <datacite:formats>
      <xsl:for-each select="$metadata//gmd:distributorFormat/*/gmd:name">
        <datacite:format>
          <xsl:value-of select="gco:CharacterString" />
        </datacite:format>
      </xsl:for-each>
  ...
  -->
  <!--<xsl:template mode="toDatacite" match="gmd:distributor[1]/*/gmd:distributorContact[1]/*">
       <datacite:publisher>
       <xsl:value-of select="gmd:organisationName/gco:CharacterString" />
       </datacite:publisher>
       </xsl:template>-->
  ...
  <xsl:template mode="toDatacite" match="gmd:identificationInfo/*/gmd:citation/*/gmd:date[1]">
    <datacite:dates>
      <xsl:for-each select="../gmd:date/*[gmd:dateType/*/@codeListValue = $dateMapping/entry/@key]">
        <xsl:variable name="key" select="gmd:dateType/*/@codeListValue" />
        <datacite:date dateType="{$dateMapping//*[@key = $key]/text()}">
          <xsl:value-of select="gmd:date/*/text()" />
        </datacite:date>
      </xsl:for-each>
    </datacite:dates>
    <xsl:for-each select="$metadata//gmd:identificationInfo/*/gmd:citation/*/gmd:date/*[gmd:dateType/*/@codeListValue = ('publication','creation')]/gmd:date[* != '']/substring(*, 1, 4)">
    ...
    <xsl:choose>
      <xsl:when test="gmd:PT_FreeText">
        <xsl:for-each select="gmd:PT_FreeText/gmd:textGroup/*">
          ...
          <xsl:variable name="languageCode" select="$metadata/gmd:locale/*[concat('#', @id) = $languageId]/gmd:languageCode/*/@codeListValue" />
          <xsl:element name="{name($template/*)}">
